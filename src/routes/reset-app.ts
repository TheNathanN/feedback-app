...
router.put('/', async (req, res) => {
  ...
  const resetFeedback = await prisma.feedback.createMany({
    data: [
      {
        id: 1,
  ...
  const resetComments = await prisma.comment.createMany({
    data: [
      {
        id: 1,
        content:
  ...
  const resetReplies = await prisma.reply.createMany({
    data: [
      {
        id: 1,
        content:
